//
//  MusicalMIDI1File Encode Tests.swift
...
@Suite struct MusicalMIDI1File_Encode_Tests {
    @Test
    func type0() {
    ...
    func encodeDP8Markers() throws {
        ...
        let dp8MarkersMIDIFile = try MusicalMIDI1File(data: kMIDI1File.dp8Markers)
        #expect(midiFile.isEqual(to: dp8MarkersMIDIFile))
        
        // test if raw data is equal
        
        let constructedData = try midiFile.rawData(as: [UInt8].self)
        #expect(constructedData == kMIDI1File.dp8Markers)
    }
}
