//
//  MusicalMIDI1File+Decoding Tests.swift
...
@Suite struct MusicalMIDI1File_Decoding_Tests {
    ...
    func midiFileParse_NonAsync() /* NOT ASYNC! */ throws {
        ...
        let midiFile = try /* NOT AWAIT! */ MusicalMIDI1File(data: kMIDI1File.dp8Markers)
        
        try #require(midiFile.chunks.count == 3)
        
        let encodedData = try /* NOT AWAIT! */ midiFile.rawData()
        #expect(encodedData.toUInt8Bytes() == kMIDI1File.dp8Markers)
    }
    ...
    func midiFileParse_Async() async throws {
        let midiFile = try await MusicalMIDI1File(data: kMIDI1File.dp8Markers)
    ...
    func midiFileParse_AsyncSequence() async throws {
        actor Receiver {
            var chunks: [Int: MusicalMIDI1File.AnyChunk] = [:]
            func addChunk(index: Int, chunk: MusicalMIDI1File.AnyChunk) { chunks[index] = chunk }
            init() { }
        }
        let receiver = Receiver()
        let midiFile = try await MusicalMIDI1File(data: kMIDI1File.dp8Markers) { fileHeader, chunkIndex, chunkCount, result in
            // check header info
        ...
        // ensure tracks are also stored in the `MIDI1File` as usual
        try #require(midiFile.chunks.count == 3)
        
        // this also ensures tracks stored within the `MIDI1File` are in the correct order
        let encodedData = try await midiFile.rawData()
        #expect(encodedData.toUInt8Bytes() == kMIDI1File.dp8Markers)
    }
    ...
    func decodeOptions_allowMultiTrackFormat0() async throws {
        ...
        await #expect(throws: MIDIFileDecodeError.self) {
            let options = MIDI1FileDecodeOptions(allowMultiTrackFormat0: false)
            let _ = try await MusicalMIDI1File(data: rawData, options: options)
        }
        
        // flag enabled
        do {
            let options = MIDI1FileDecodeOptions(allowMultiTrackFormat0: true)
            let midiFile = try await MusicalMIDI1File(data: rawData, options: options)
    ...
    func chunkDecodeOptions_errorStrategy_malformedEvent_invalidStatusByte() async throws {
        ...
        // throwOnError
        await #expect(throws: MIDIFileDecodeError.self) {
            let options = MIDI1FileDecodeOptions(chunkDecodeOptions: .init(errorStrategy: .throwOnError))
            let _ = try await MusicalMIDI1File(data: rawData, options: options)
        }
        
        // discardTracksWithErrors
        do {
            let options = MIDI1FileDecodeOptions(chunkDecodeOptions: .init(errorStrategy: .discardTracksWithErrors))
            let midiFile = try await MusicalMIDI1File(data: rawData, options: options)
        ...
        // allowLossyRecovery
        do {
            let options = MIDI1FileDecodeOptions(chunkDecodeOptions: .init(errorStrategy: .allowLossyRecovery))
            let midiFile = try await MusicalMIDI1File(data: rawData, options: options)
    ...
    func predicate() /* NOT ASYNC! */ throws {
        // Note: It's ok if this throws a deprecation warning. We need to test this specific method.
        let midiFile = try /* NOT AWAIT! */ MusicalMIDI1File(data: kMIDI1File.dp8Markers) { identifier, chunkIndex in
    ...
    func predicate_async() async throws {
        let midiFile = try await MusicalMIDI1File(data: kMIDI1File.dp8Markers) { identifier, chunkIndex in
            identifier == .track && chunkIndex == 1
        }
        try #require(midiFile.chunks.count == 1)
        let track = midiFile.tracks[0]
    ...
    func decodeOptions_bundleRPNAndNRPNEvents() async throws {
        ...
        do {
            let options = MIDI1FileDecodeOptions(chunkDecodeOptions: .init(bundleRPNAndNRPNEvents: false))
            let decodedMIDIFile = try await MusicalMIDI1File(data: rawData, options: options)
        ...
        do {
            let options = MIDI1FileDecodeOptions(chunkDecodeOptions: .init(bundleRPNAndNRPNEvents: true))
            let decodedMIDIFile = try await MusicalMIDI1File(data: rawData, options: options)
            #expect(decodedMIDIFile.tracks.count == 1)
            let decodedTrack = decodedMIDIFile.tracks[0]
    ...
    func decodeSpuriousBytesPastEOF_allowLossyRecovery() async throws {
        ...
        let garbageChunk: MusicalMIDI1File.AnyChunk = .undefined(identifier: "\u{43}\u{12}\u{01}\u{78}", data: Data())!
        var salvagedMIDIFile = try await MusicalMIDI1File(data: kMIDI1File.customChunk)
        ...
        do {
            let options = MIDI1FileDecodeOptions(ignoreBytesPastEOF: false, chunkDecodeOptions: .init(errorStrategy: .allowLossyRecovery))
            
            let rawDataCR = baseRawData + [0x0D]
            await #expect(throws: (any Error).self) { try await MusicalMIDI1File(data: rawDataCR, options: options) }
            
            let rawDataLF = baseRawData + [0x0A]
            await #expect(throws: (any Error).self) { try await MusicalMIDI1File(data: rawDataLF, options: options) }
            
            let rawDataCRLF = baseRawData + [0x0D, 0x0A]
            await #expect(throws: (any Error).self) { try await MusicalMIDI1File(data: rawDataCRLF, options: options) }
            
            let rawDataCRLF_CRLF = baseRawData + [0x0D, 0x0A]
            await #expect(throws: (any Error).self) { try await MusicalMIDI1File(data: rawDataCRLF_CRLF, options: options) }
            
            let rawDataSpace = baseRawData + [0x20] // space char
            await #expect(throws: (any Error).self) { try await MusicalMIDI1File(data: rawDataSpace, options: options) }
    ...
    func decodeSpuriousBytesPastEOF_throwOnError() async throws {
        ...
        // (throwing error if spurious trailing bytes are encountered)
        do {
            let options = MIDI1FileDecodeOptions(ignoreBytesPastEOF: false, chunkDecodeOptions: .init(errorStrategy: .throwOnError))
            ...
            let rawData8Bytes = baseRawData + [0x43, 0x12, 0x01, 0x78, 0x00, 0x09, 0x10, 0x58]
            await #expect(throws: (any Error).self) { try await MusicalMIDI1File(data: rawData8Bytes, options: options) }
        }
    }
    ...
    func decodeMoreTracksThanSpecifiedInHeader() async throws {
        ...
        // ignoring spurious trailing bytes
        do {
            let options = MIDI1FileDecodeOptions(ignoreBytesPastEOF: true)
        ...
        do {
            let options = MIDI1FileDecodeOptions(ignoreBytesPastEOF: false)
            let decodedMIDIFile = try await MusicalMIDI1File(data: rawDataExtraTrack, options: options)
