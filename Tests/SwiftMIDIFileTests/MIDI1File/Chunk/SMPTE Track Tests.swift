...
@Suite
struct SMPTE_Track_Tests {
    // swiftformat:options --wrapcollections preserve
    // swiftformat:disable spaceInsideParens spaceInsideBrackets spacearoundoperators
    // swiftformat:options --maxwidth none

    @Test
    func eventsAtTimecodeLocations() throws {
        let tpf: UInt8 = 40
        let timebase: SMPTEMIDI1File.Timebase = .smpte(frameRate: .fps25, ticksPerFrame: tpf)
        var midiFile = SMPTEMIDI1File(timebase: timebase)
