...
struct SMPTE_DeltaTime_Tests {
    ...
    @Test
    func smpteStaticConstructors_fromSMPTEOffset_25fps_20tpf() {
        ...
        #expect(Delta.offset(MIDIFileEvent.SMPTEOffset(hr: 00, min: 01, sec: 20, fr: 10, rate: .fps25)).ticks(using: timebase) == 40200)
        #expect(Delta.offset(MIDIFileEvent.SMPTEOffset(hr: 01, min: 01, sec: 20, fr: 10, rate: .fps25)).ticks(using: timebase) == 1_840_200)
    }

    @Test
    func smpteStaticConstructors_fromSMPTEOffset_25fps_40tpf() {
        typealias Delta = SMPTEMIDI1File.Track.DeltaTime
        let timebase: Delta.Timebase = .smpte(frameRate: .fps25, ticksPerFrame: 40)

        #expect(Delta.offset(MIDIFileEvent.SMPTEOffset(hr: 00, min: 00, sec: 00, fr: 00, rate: .fps25)).ticks(using: timebase) == 0)
        #expect(Delta.offset(MIDIFileEvent.SMPTEOffset(hr: 00, min: 01, sec: 20, fr: 10, rate: .fps25)).ticks(using: timebase) == 80400)
        #expect(Delta.offset(MIDIFileEvent.SMPTEOffset(hr: 01, min: 01, sec: 20, fr: 10, rate: .fps25)).ticks(using: timebase) == 3_680_400)
    }

    @Test
    func smpteStaticConstructors_frames_25fps_20tpf() {
        typealias Delta = SMPTEMIDI1File.Track.DeltaTime
        let timebase: Delta.Timebase = .smpte(frameRate: .fps25, ticksPerFrame: 20)
    ...
    @Test
    func smpteStaticConstructors_frames_25fps_40tpf() {
        typealias Delta = SMPTEMIDI1File.Track.DeltaTime
        let timebase: Delta.Timebase = .smpte(frameRate: .fps25, ticksPerFrame: 40)
    ...
    @Test
    func smpteStaticConstructors_offset_25fps_20tpf() {
        typealias Delta = SMPTEMIDI1File.Track.DeltaTime
        let timebase: Delta.Timebase = .smpte(frameRate: .fps25, ticksPerFrame: 20)

        #expect(Delta.offset(hr: 00, min: 00, sec: 00, fr: 00, rate: .fps25).ticks(using: timebase) == 0)
        #expect(Delta.offset(hr: 00, min: 01, sec: 20, fr: 10, rate: .fps25).ticks(using: timebase) == 40200)
        #expect(Delta.offset(hr: 01, min: 01, sec: 20, fr: 10, rate: .fps25).ticks(using: timebase) == 1_840_200)
    }

    @Test
    func smpteStaticConstructors_offset_25fps_40tpf() {
        typealias Delta = SMPTEMIDI1File.Track.DeltaTime
        let timebase: Delta.Timebase = .smpte(frameRate: .fps25, ticksPerFrame: 40)

        #expect(Delta.offset(hr: 00, min: 00, sec: 00, fr: 00, rate: .fps25).ticks(using: timebase) == 0)
        #expect(Delta.offset(hr: 00, min: 01, sec: 20, fr: 10, rate: .fps25).ticks(using: timebase) == 80400)
        #expect(Delta.offset(hr: 01, min: 01, sec: 20, fr: 10, rate: .fps25).ticks(using: timebase) == 3_680_400)
    }

    @Test
    func smpteStaticConstructors_offset_edgeCases_25fps_0tpf() {
        typealias Delta = SMPTEMIDI1File.Track.DeltaTime
    ...
    @Test
    func smpteStaticConstructors_offset_edgeCases_25fps_1tpf() {
        typealias Delta = SMPTEMIDI1File.Track.DeltaTime
        ...
        #expect(Delta.offset(hr: 01, min: 01, sec: 20, fr: 10, rate: .fps25).ticks(using: timebase) == 92010)
    }

    @Test
    func smpteStaticConstructors_offset_edgeCases_25fps_255tpf() {
        typealias Delta = SMPTEMIDI1File.Track.DeltaTime
