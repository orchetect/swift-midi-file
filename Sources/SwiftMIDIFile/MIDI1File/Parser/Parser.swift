...
extension MIDI1File.Parser: Sendable { }
...
extension MIDI1File.Parser {
    ...
    func chunks(
        ...
        var newChunks: [MIDI1File.AnyChunk] = []
    ...
    func chunksAsyncSequence(
        options: MIDI1FileDecodeOptions,
        predicate: MIDI1File.DecodePredicate?
    ) -> AsyncStream<(index: Int, result: Result<MIDI1File.AnyChunk, MIDIFileDecodeError>)> {
        AsyncStream { continuation in
            Task { [data, options, predicate, continuation] in
                await withTaskGroup(
                    of: (index: Int, result: Result<MIDI1File.AnyChunk, MIDIFileDecodeError>)?.self,
                    returning: Void.self
