...
Converting `MIDIFileEvent` → `MIDIEvent`:

- ``midiEvent()``

The remainder of `MIDIFileEvent` event types are only relevant to MIDI files and have no MIDI I/O event equivalent.

For more information on `MIDIEvent`, refer to its documentation in the SwiftMIDICore package.
...
- <doc:MIDIFileEvent-RPN>
- <doc:MIDIFileEvent-NRPN>
