...
A quick guide on getting started using SwiftMIDIFile.

![swift-midi-file](swift-midi-file-banner.png)

This is a basic guide intended to give the most essential information on getting set up and running with SwiftMIDIFile.

There are many more features and aspects to the library that can be discovered through inline documentation.

## Import the Library

Add SwiftMIDIFile to your project, and import it.
...
> Note:
>
> Nearly all software manufacturers exclusively implement musical timebase. SMPTE timebase is extremely rare and almost never used. However, since both are defined in the specification, both are supported by SwiftMIDI.
>
...
case let .musical(midiFile):
    // midiFile is `MIDI1File<MusicalMIDIFileTimebase>`, 
    // typealiased as `MusicalMIDI1File`
case let .smpte(midiFile):
    // midiFile is `MIDI1File<SMPTEMIDIFileTimebase>`,
    // typealiased as `SMPTEMIDI1File`
}
```
...
See the **MIDI File Editor** example project in the Examples folder for a demonstration of reading and writing MIDI files.
